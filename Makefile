...
cross-build:
ifeq ($(OS),Windows_NT)
	@echo "Cross-compilation script is for Linux only. Use 'make build' instead."
else
...
help:
	...
	@echo "  install       - Install necessary packages and configure Rust targets (Linux only)"
	@echo "  build-x86_64  - Build for 64-bit Windows (Linux only)"
	@echo "  build-i686    - Build for 32-bit Windows (Linux only)"
	@echo "  cross-build   - Run cross-compilation script (Linux only)"
